...
H5FD__hermes_term(void)
{
    ...
    if (H5FDhermes_err_class_g >= 0) {
        if (H5Eunregister_class(H5FDhermes_err_class_g) < 0)
            H5FD_HERMES_GOTO_ERROR(H5E_VFL, H5E_CLOSEERROR, FAIL, "can't unregister error class from HDF5 error API");
        ...
        if (H5Eclose_stack(H5FDhermes_err_stack_g) < 0) {
            H5FD_HERMES_GOTO_ERROR(H5E_VFL, H5E_CLOSEERROR, FAIL, "can't close HDF5 error stack");
            PRINT_ERROR_STACK;
        } /* end if */
...
H5Pset_fapl_hermes(hid_t fapl_id, hbool_t persistence, size_t page_size)
{
    ...
    if (H5I_GENPROP_LST != H5Iget_type(fapl_id) || TRUE != H5Pisa_class(fapl_id, H5P_FILE_ACCESS))
        H5FD_HERMES_GOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not a file access property list");
    ...
    if (H5Pset_driver(fapl_id, H5FD_HERMES, &fa) < 0)
        H5FD_HERMES_GOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL, "can't set Hermes VFD as driver");
...
H5FD__hermes_open(const char *name, unsigned flags, hid_t fapl_id, haddr_t maxaddr)
{
    ...
    if ((H5OPEN hermes_initialized) == FAIL) {
        ...
        if (HermesInitHermes(hermes_config) < 0) {
            H5FD_HERMES_GOTO_ERROR(H5E_SYM, H5E_UNINITIALIZED, NULL, "Hermes initialization failed");
        }
        else {
    ...
    if (NULL == (file = calloc(1, sizeof(H5FD_hermes_t))))
        H5FD_HERMES_GOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL, "unable to allocate file struct");
    ...
    if (fa->persistence) {
        ...
        if (fstat(fd, &sb) < 0)
            H5FD_HERMES_SYS_GOTO_ERROR(H5E_FILE, H5E_BADFILE, NULL, "unable to fstat file");
...
H5FD__hermes_close(H5FD_t *_file)
{
    ...
    if (file->persistence) {
        ...
        if (close(file->fd) < 0)
            H5FD_HERMES_SYS_GOTO_ERROR(H5E_IO, H5E_CANTCLOSEFILE, FAIL, "unable to close file");
    }
    if (file->ref_count == 1)
        HermesBucketDestroy(file->bkt_handle);
    else
                ...
                size_t size, void *buf /*out*/)
{
    ...
    if (HADDR_UNDEF == addr)
        H5FD_HERMES_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL, "addr undefined, addr = %llu", (unsigned long long)addr);
    if (REGION_OVERFLOW(addr, size))
        H5FD_HERMES_GOTO_ERROR(H5E_ARGS, H5E_OVERFLOW, FAIL, "addr overflow, addr = %llu", (unsigned long long)addr);
    if (NULL == file->page_buf)
        H5FD_HERMES_GOTO_ERROR(H5E_INTERNAL, H5E_UNINITIALIZED, FAIL, "transfer buffer not initialized");
                 ...
                 size_t size, const void *buf)
{
    ...
    if (HADDR_UNDEF == addr)
        H5FD_HERMES_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL, "addr undefined, addr = %llu", (unsigned long long)addr);
    if (REGION_OVERFLOW(addr, size))
        H5FD_HERMES_GOTO_ERROR(H5E_ARGS, H5E_OVERFLOW, FAIL, "addr overflow, addr = %llu, size = %llu",
